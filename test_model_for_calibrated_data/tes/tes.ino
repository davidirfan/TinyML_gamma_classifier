...
// Parses a CSV row in the format "float1,float2,float3" and stores it in inputData
void parseAndStoreRow(String line) {
  ...
  if (firstComma == -1 || secondComma == -1) {
    Serial.println("Invalid input format. Expected: float1,float2,float3");
    return;
  }

  float val1 = line.substring(0, firstComma).toFloat();
  float val2 = line.substring(firstComma + 1, secondComma).toFloat();
  ...
  Serial.printf("Stored row %d: %.2f, %.2f, %.2f\n", rowIndex, val1, val2, val3);
  rowIndex++;
}
...
void showOutput() {
  ...
  for (int i = 0; i < rowIndex; i++) {
    Serial.printf("[%d] %.2f, %.2f, %.2f, %.2f \n", i, inputData[i][0], inputData[i][1], inputData[i][2], outputData[i][0]);
